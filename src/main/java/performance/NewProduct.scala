...
class NewProduct extends Simulation {

	val httpProtocol = http
		.baseUrl("http://www.dp2.com")
		.inferHtmlResources(BlackList(""".*.ico""", """.*.css""", """.*.js""", """.*.png""",""".*.gif"""), WhiteList())
  ...
  object LoginAsAdmin {
	...
	object NewProductAdmin{
		val newProduct = exec(http("NewProduct")
			.get("/products/new")
			.headers(headers_0)
			.check(css("input[name=_csrf]", "value").saveAs("stoken"))
		).pause(20)
		.exec(http("ProductListAfter")
			.post("/products/save")
			.headers(headers_3)
			.formParam("name", "prueba1")
			.formParam("description", "prueba1")
			.formParam("price", "45")
			.formParam("stock", "45")
			.formParam("urlImage", "http://www.dp2.com/products/new")
			.formParam("id", "")
			.formParam("_csrf", "${stoken}"))
		.pause(17)
	}
	...
	val ownerScn = scenario("OwnerNewProducts").exec(
		Home.home,
	...
	val adminScen = scenario("AdminNewProducts").exec(
		Home.home,
		LoginAsAdmin.loginAsAdmin,
		ProductListBefore.productListBefore,
		NewProductAdmin.newProduct
	)

	setUp(
		ownerScn.inject(rampUsers(500) during (100 seconds)),
		adminScen.inject(rampUsers(500) during (100 seconds)))
